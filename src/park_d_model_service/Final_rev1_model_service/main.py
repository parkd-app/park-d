...
#Judge availability of the parking spot by using midpoint
...
# Set up model, loading weights and configuration
def set_up_model():
    cfg = "yolov3.cfg"
...
#Commit changes to db for real time parking spot information
...
def main_for_rev1():
    ...
    prev_url = ""
    while True:
        #Check the current input of the model
        url = query_by_key("current_input")
        if prev_url != url:
            print("input changes!!!")
            prev_url = url[:]
        ...
        #Sleep 5 seconds and start the next round 
        time.sleep(5)
